...
# VERSION: v6.3 Masterpiece (All-in-One)
# AUTHOR: HASSAN K3KO
# =========================================
...
while true; do
    ...
    echo -e "${C_PRP}║${C_NC}${C_CYN}               [ PROFESSIONAL VPS MANAGER v6.3 ]            ${C_NC}${C_PRP}║${C_NC}"
    echo -e "${C_PRP}╠════════════════════════════════════════════════════════════╣${C_NC}"
    ...
    echo -e "${C_PRP}╠════════════════════════════════════════════════════════════╣${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[1]${C_NC} 🚀 Install All (SSL + WebSocket + SSH Setup)       ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[2]${C_NC} 👤 Add SSH / VPN User (Days & Max Limit)           ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[3]${C_NC} ❌ Delete SSH User                                 ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[4]${C_NC} 📋 List Users, Expiry & Active Connections         ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[5]${C_NC} ⚙️ Domain, Ports & Banner Settings                 ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}║${C_NC}  ${C_GRN}[6]${C_NC} 🔄 Update Script from Web (GitHub)                 ${C_PRP}║${C_NC}"
    echo -e "${C_PRP}╚════════════════════════════════════════════════════════════╝${C_NC}"
    ...
    case $choice in
        1)
            clear
            echo -e "${C_YLW}--- INSTALLING ALL SERVICES (SSL + WS + SSH) ---${C_NC}"
            apt-get update -y >/dev/null 2>&1
            apt-get install stunnel4 python3 python3-pip ufw iptables -y >/dev/null 2>&1
            
            # إعداد SSL (Stunnel4) على البورت 443
...
EOF
            ...
            # إعداد WebSocket Proxy على البورت 80
...
EOF
            ...
            # فتح جميع البورتات اللازمة تلقائياً
            ...
            echo -e "${C_GRN}==================================================${C_NC}"
            echo -e "${C_GRN} All Services Installed & Configured Successfully! ${C_NC}"
            echo -e "${C_GRN} - SSL Port : 443                                 ${C_NC}"
            echo -e "${C_GRN} - WS Port  : 80                                  ${C_NC}"
            echo -e "${C_GRN} - SSH Port : 22                                  ${C_NC}"
            echo -e "${C_GRN}==================================================${C_NC}"
            read -p "Press Enter to continue..."
            ;;
        2)
            clear
            echo -e "${C_GRN}--- ADD NEW USER ---${C_NC}"
        ...
        5)
            clear
            echo -e "${C_YLW}--- SETTINGS: DOMAIN, PORTS & BANNER ---${NC}"
            echo "1. Set / Change Domain"
            echo "2. Set / Change SSH Banner"
            echo "3. Open ALL Ports (1 to 65535)"
