...
while true; do
    clear
    [ -f /etc/os-release ] && . /etc/os-release && SYS_OS="$PRETTY_NAME" || SYS_OS="Linux"
    ...
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${YELLOW}    _  __ _____ _          _           ____            ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${YELLOW}   | |/ /| ____| |ovalent / /_   __  __/ ___|  ___ _ __ ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${YELLOW}   | ' / |  _| | |/ / _ \ '_ \ | '_ \| |     / __| '__|${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${YELLOW}   | . \ | |___|   <  __/ (_) || |_) | |___ | (__| |   ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${YELLOW}   |_|\_\|_____|_|\_\___|\___/ | .__/ \____(_)___|_|   ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC} ${BOLD}${WHITE}               [ PROFESSIONAL VPS SUITE v4.0 ]               ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${NC} ${BLUE}🌐 IP:${NC} ${WHITE}$PUBLIC_IP${NC}  ${CYAN}│${NC} ${BLUE}💻 OS:${NC} ${WHITE}$SYS_OS${NC}"
    echo -e "${CYAN}║${NC} ${BLUE}🔗 DOMAIN:${NC} ${CYAN}$DOMAIN${NC}"
    echo -e "${CYAN}║${NC} ${BLUE}👥 ACTIVE:${NC} ${GREEN}$ONLINE_USERS Online${NC}  ${CYAN}│${NC} ${BLUE}⏳ RUNNING:${NC} ${GREEN}$VPS_DAYS Days${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${NC} ${YELLOW}                 --- SERVER MANAGEMENT ---                  ${NC} ${CYAN}║${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${NC}  ${GREEN}[1]${NC} 🔑 SSH / OVPN Manager    ${GREEN}[4]${NC} 🛡️ Trojan Manager     ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  ${GREEN}[2]${NC} ⚡ VMess Manager         ${GREEN}[5]${NC} 📦 Shadowsocks Manager ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  ${GREEN}[3]${NC} 🚀 VLESS Manager         ${GREEN}[6]${NC} ⚙️ All Ports & Settings ${CYAN}║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
    echo -e "                  ${WHITE}System Date: $(date '+%Y-%m-%d')${NC}"
    echo ""
